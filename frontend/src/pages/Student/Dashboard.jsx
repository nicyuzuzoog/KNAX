...
import { 
  FaHome, FaClipboardList, FaCalendarCheck, FaClock, FaCheckCircle, FaTimesCircle,
  FaHourglassHalf, FaUserGraduate, FaBuilding, FaBook, FaUser, FaCog, FaSignOutAlt,
  FaBars, FaTimes, FaEdit, FaLock, FaEnvelope, FaPhone, FaTasks, FaBell, FaGraduationCap
} from 'react-icons/fa';
...
const StudentDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Safe defaults
      const regData = regRes?.data || null;
...
// ============================================
// OverviewSection example fix:
const OverviewSection = ({ user, registration, attendance, fallbackImages, departmentImages }) => {
  const today = new Date();
  const currentDept = registration?.department || user?.department || null;
  ...
  const calculateAttendanceStats = () => {
    const total = Array.isArray(attendance) ? attendance.length : 0;
